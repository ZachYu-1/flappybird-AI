...
from neat_para import generation, prob_threshold_to_jump, failed_punishment
...
def main(genomes, config):
    ...
    pipes_list = [Pipe(pipe_starting_x_position + i * pipe_horizontal_gap) for i in
                  range(pipe_max_num)]  # build the pipes and separate them by pipe_horizontal_gap
    ...
    while run is True:  # when we run the program
        ...
        score = len(passed_pipes)  # score of the game equals to the number of pipes the bird passed
        ...
        for index, bird in enumerate(birds_list):
            ...
            if output[0] > prob_threshold_to_jump:  # if the output is greater than the probability threshold to jump
